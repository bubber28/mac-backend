// api/index.js - Versão Completa Restaurada
...
// ==================== FUNÇÕES AUXILIARES (todas preservadas) ====================
...
function tokenizarTexto(texto = "") {
  ...
  return normalizarTexto(texto).split(" ").filter(t => t && t.length > 1 && !stopWords.has(t));
}
...
function detectarItensPorMensagem(servicos = [], mensagem = "") {
  ...
  return servicos.map(servico => {
    ...
    let scoreNome = 0, scoreDescricao = 0;
    ...
    if (mensagemNormalizada.includes("combo") && (nomeNormalizado.includes("combo") || descricaoNormalizada.includes("combo") || descricaoNormalizada.includes("festa") || descricaoNormalizada.includes("evento"))) scoreNome += 3;
  ...
  }).filter(item => item.score_total >= 2).sort((a, b) => b.score_total - a.score_total);
}
...
async function salvarAnaliseConversa(leadId, analiseMensagem) {
  ...
  if (error) console.error("Erro salvar análise:", error.message);
}
...
async function buscarPerfilLead(leadId) {
  ...
  const { data } = await supabase.from("perfil_lead_mac").select("*").eq("lead_id", leadId).maybeSingle();
  return data;
}
...
app.post("/chat", async (req, res) => {
  try {
    ...
    const resposta = "Olá! Recebi sua mensagem. O M.A.C. está funcionando. Em breve a análise completa estará restaurada.";

    res.json({
      ok: true,
      lead_id: "temp",
      resposta: resposta,
      origem_resposta: "modo_diagnostico"
    });
  } catch (err) {
